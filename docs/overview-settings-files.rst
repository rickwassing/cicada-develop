...
.. note::
    ...
    - The WASO Events are defined by the combination of ``awakenings`` (*N*) and ``waso``, such that there are *N* WASO events, each with a duration of ``waso`` / ``awakenings`` minutes.
