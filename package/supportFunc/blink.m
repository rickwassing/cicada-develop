...
if isempty(t) % No timer, set one up
    ...
    setappdata(h,'OriginalFontColor', h.FontColor)
...
CurrentFontColor = h.FontColor;
OriginalFontColor = getappdata(h, 'OriginalFontColor');
if isequaln(CurrentFontColor, OriginalFontColor) % Matches original text
    h.FontColor = [0.86, 0.21, 0.27];
else
