function [dateTime, err, msg] = parseSleepDiaryDateTime(thisDate, thisTime, format)

% Set 'err' to false, i.e. assume all is well
err = false;
msg = '';

% If the string 'thisTime' includes a day, month, and a year, then we can parse
...
if ...
    ...
    try
        ...
        err = true;
        msg = 'Error in parseSleepDiaryDateTime (line 21). Date-time formatting failed';
    end
...
% refers to a sleep event that happend the same day as the diary was completed.
    ...
    try
        ...
        dateTime = nan(size(thisTime));
        err = true;
        msg = 'Error in parseSleepDiaryDateTime (lines 52 or 54). Date-time formatting failed';
    end
end