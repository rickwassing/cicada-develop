...
idxSleepRows = strcmpi(app.ACT.stats.sleep.actigraphy.type, 'sleep');
% ---------------------------------------------------------
% Sleep overview
% ---------------------------------------------------------
% UIAxes
...
if shouldComponentMount(app, GridLayoutContainer, 'SleepStats_UIAxesOverview')
    ...
    props = { ...
        ...
        'XLim', [0.5, max([sum(idxSleepRows), 14])+0.5]; ...
        'XTick', 1:sum(idxSleepRows); ...
        'XTickLabel', cellfun(@(pre, post) [titleCase(pre), ' #', post], app.ACT.stats.sleep.actigraphy.type(idxSleepRows), cellstr(num2str(app.ACT.stats.sleep.actigraphy.count(idxSleepRows))), 'UniformOutput', false); ...
        'YDir', 'reverse'; ...
        'YGrid', 'on'; ...
        'GridAlpha', 0.33; ...
...
for si = 1:size(app.ACT.stats.sleep.actigraphy, 1)
    if ~strcmpi(app.ACT.stats.sleep.actigraphy.type{si}, 'sleep')
        continue
    end
    cnt = cnt+1;
    % -----
    ...
    % -----
    % Extract YData
    y    = mod(datenum(app.ACT.stats.sleep.actigraphy.clockEventOnset{si}, 'dd/mm/yyyy HH:MM'), 1)*24;
    y    = ifelse(y < parent.YLim(1), y+24, y);
    y(2) = mod(datenum(app.ACT.stats.sleep.actigraphy.clockEventOffset{si}, 'dd/mm/yyyy HH:MM'), 1)*24;
    ...
    if shouldComponentMount(app, parent, ['SleepStats_PatchOverview_sleepWindow-', num2str(si)])
        ...
        props = {...
            ...
            'XData', [cnt-0.25, cnt-0.25, cnt+0.25, cnt+0.25]; ...
            'YData', [y, fliplr(y)]; ...
    ...
    XData = repmat([cnt-0.25, cnt+0.25, cnt+0.25, cnt-0.25], 1, size(events, 1));
    % -----
    % Check if component should mount
...
if shouldComponentMount(app, parent, 'SleepStats_GridLayoutContainerPanel')
    ...
    props = {...
        ...
        'ColumnWidth', {'1x'}; ...
        'RowHeight', repmat({120}, 1, sum(idxSleepRows)); ...
        'ColumnSpacing', 0; ...
        'RowSpacing', 3; ...
        'Padding', [0, 0, 0, 0]; ...
        'Scrollable', 'on'; ...
...
for si = 1:size(app.ACT.stats.sleep.actigraphy, 1)
    ...
    if shouldComponentMount(app, SleepStats_GridLayoutContainerPanel, ['SleepStats_Panel-', num2str(si)])
        ...
        props = {...
            ...
            'Title', [ifelse(strcmpi(app.ACT.stats.sleep.actigraphy.type{si}, 'sleep'), 'Sleep', 'Nap'), ' Window #', num2str(app.ACT.stats.sleep.actigraphy.count(si))]; ...
            'FontSize', 8; ...
            'FontWeight', 'bold'; ...
            'Layout', Layout; ...
    ...
    if shouldComponentMount(app, parent, ['SleepStats_GridLayout-', num2str(si)])
        ...
        props = {...
            ...
            'ColumnWidth', [{100}, {35}, repmat({'1x'}, 1, 16)]; ...
    ...
    if isfield(app.ACT.stats.sleep, 'sleepDiary')
        ...
        onset  = datenum(app.ACT.stats.sleep.sleepDiary.clockEventOnset, 'dd/mm/yyyy HH:MM');
        offset = datenum(app.ACT.stats.sleep.sleepDiary.clockEventOffset, 'dd/mm/yyyy HH:MM');
        idx = ...
            (...
            onset >= datenum(app.ACT.stats.sleep.actigraphy.clockEventOnset{si}, 'dd/mm/yyyy HH:MM') & ...
            onset <= datenum(app.ACT.stats.sleep.actigraphy.clockEventOffset{si}, 'dd/mm/yyyy HH:MM') ...
            ) | (...
            offset >= datenum(app.ACT.stats.sleep.actigraphy.clockEventOnset{si}, 'dd/mm/yyyy HH:MM') & ...
            offset <= datenum(app.ACT.stats.sleep.actigraphy.clockEventOffset{si}, 'dd/mm/yyyy HH:MM') ...
            ) | (...
            onset <= datenum(app.ACT.stats.sleep.actigraphy.clockEventOnset{si}, 'dd/mm/yyyy HH:MM') & ...
            offset >= datenum(app.ACT.stats.sleep.actigraphy.clockEventOffset{si}, 'dd/mm/yyyy HH:MM') ...
            );
    ...
    try
        SleepWindowAct = [datenum(app.ACT.stats.sleep.actigraphy.clockEventOnset{si}, 'dd/mm/yyyy HH:MM'), datenum(app.ACT.stats.sleep.actigraphy.clockEventOffset{si}, 'dd/mm/yyyy HH:MM')];
    catch
        SleepWindowAct = [];
    end
    try
    ...
    try
        SleepWindowDiary = [datenum(app.ACT.stats.sleep.sleepDiary.clockEventOnset{idx}, 'dd/mm/yyyy HH:MM'), datenum(app.ACT.stats.sleep.sleepDiary.clockEventOffset{idx}, 'dd/mm/yyyy HH:MM')];
    catch
        SleepWindowDiary = [];
    end
    try
