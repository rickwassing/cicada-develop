...
if isfield(app.ACT.analysis.annotate, 'acceleration')
    % ... Plot the annotation data for each intensity level
    for intensity = 4:-1:3
        % Get properties that need more elaborate calculation
        [XData, YData, Color] = app_getAnnotationPatchProps(app, 'acceleration', intensity, 4, app.ACT.xmin, app.ACT.xmax);
        addToYData = ceil(XData) - floor(XData(1));
        XData = mod(XData, 1)*24;
        YData = -map(YData, ...
            min(YData), max(YData), ...
            -0.33, -0.24, true) + addToYData;
        for idx = 2:4:length(XData)-1
            if shouldComponentMount(app, parent, ['DailyStats_PatchOverviewAnnot_int-' num2str(intensity), '_idx-' num2str(idx)])
                % Define the properties
                props = { ...
                    'Tag', ['DailyStats_PatchOverviewAnnot_int-' num2str(intensity), '_idx-' num2str(idx)]; ...
                    'XData', XData(idx:idx+3); ...
                    'YData', YData(idx:idx+3); ...
                    'FaceColor', Color; ...
                    'LineStyle', '-'; ...
                    'LineWidth', 1; ...
                    'EdgeColor', Color; ...
                ...
                constructComponent(app, ['DailyStats_PatchOverviewAnnot_int-' num2str(intensity), '_idx-' num2str(idx)], parent, { ...
                    'XData', XData(idx:idx+3); ...
                    'YData', YData(idx:idx+3); ...
                    'FaceColor', Color; ...
                    'EdgeColor', Color; ...
        ...
        if ~strcmpi(XData, 'na')
            ...
            if shouldComponentMount(app, parent, ['DailyStats_Plot' varName{:} 'EuclNormMovWin5h_day-', num2str(di)])
                ...
                props = {...
                    ...
                    'LineStyle', '-'; ...
                    'LineWidth', 1; ...
                    'Marker', ifelse(strcmpi(varName{:}, 'Min'), 'v', '^'); ...
                    'Color', ifelse(strcmpi(varName{:}, 'Min'), [0.0936, 0.3909, 0.6030], [0.4983, 0.0222, 0.1202]); ...
                    'MarkerFaceColor', ifelse(strcmpi(varName{:}, 'Min'), [0.3059, 0.5805, 0.7765], [0.7059, 0.1490, 0.2636]); ...
                    'MarkerEdgeColor', ifelse(strcmpi(varName{:}, 'Min'), [0.0936, 0.3909, 0.6030], [0.4983, 0.0222, 0.1202]); ...
                    };
            ...
            % -----
            % Check if component should mount
            if shouldComponentMount(app, parent, ['DailyStats_Text' varName{:} 'EuclNormMovWin5h_day-', num2str(di)])
                % Define the properties
                props = { ...
                    'Tag', ['DailyStats_Text' varName{:} 'EuclNormMovWin5h_day-', num2str(di)]; ...
                    'FontSize', 9; ...
                    'FontWeight', 'bold'; ...
                    'Color', 'w'; ...
                    'VerticalAlignment', ifelse(strcmpi(varName{:}, 'Min'), 'bottom', 'top'); ...
                    'HorizontalAlignment', 'center'; ...
                    'BackgroundColor', ifelse(strcmpi(varName{:}, 'Min'), [0.0936, 0.3909, 0.6030], [0.4983, 0.0222, 0.1202]); ...
                ...
                constructComponent(app, ['DailyStats_Text' varName{:} 'EuclNormMovWin5h_day-', num2str(di)], parent, { ...
                    'String', datestr(XData(4)/24, 'HH:MM'); ...
                    'Position', [XData(4), YData(4), 0]; ...
                    });
            end
