...
if parseErr
    err = true;
    msg = sprintf('Error in cic_parseSleepDiary (line 13), could not parse ''date'' as ''%s''', importSettings.format.date);
end
% ---------------------------------------------------------
...
if parseErr
    err = true;
    msg = sprintf('Error in cic_parseSleepDiary (line 22), could not parse ''lights out'' as ''%s''', importSettings.format.lightsOut);
end
% ---------------------------------------------------------
...
try
    ...
    err = true;
    msg = 'Error in cic_parseSleepDiary (line 31), could not parse ''sleep latency'' as ''integer''';
end
% ---------------------------------------------------------
...
try
    ...
    err = true;
    msg = 'Error in cic_parseSleepDiary (line 40), could not parse ''awakenings'' as ''integer''';
end
% ---------------------------------------------------------
...
try
    ...
    err = true;
    msg = 'Error in cic_parseSleepDiary (line 49), could not parse ''waso'' as ''integer''';
end
% ---------------------------------------------------------
% Try to parse the final awakening date-time, we assume a datestring with at least the hours and minutes
[ACT.analysis.sleepDiary.finAwake, parseErr] = parseSleepDiaryDateTime(ACT.analysis.sleepDiary.date, rawTable.(items{importSettings.idx.finAwake}), importSettings.format.finAwake);
if parseErr
    err = true;
    msg = sprintf('Error in cic_parseSleepDiary (line 58), could not parse ''final awakening'' as ''%s''', importSettings.format.finAwake);
end
% ---------------------------------------------------------
...
if parseErr
    err = true;
    msg = sprintf('Error in cic_parseSleepDiary (line 67), could not parse ''lights on'' as ''%s''', importSettings.format.lightsOn);
end
