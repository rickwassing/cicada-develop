...
inputTemp = inputTemp - mean(inputTemp(:,1), 'omitnan');
...
for iter = 1:maxIter
    ...
    % Date: Aug 2022
    % By: Rick Wassing
    % Reason: the residuals were not averaged correctly resulting in an
    % inaccurate threshold to break out of the loop. Now validated that
    % this code produces the same values as the code in 'g.calibrate.R'.
    residuals = [residuals, 3 * mean(mean(repmat(weights', 1, 3) .* (curr - closestPoint).^2 / sum(weights)))]; %#ok<AGROW>
    weights   = min([1 ./ sqrt(sum((curr-closestPoint).^2,2)), ones(size(curr,1),1).*100],[],2)';
...
ACT.etc.cal.temp_x_offset = tempOffset(1);
ACT.etc.cal.temp_y_offset = tempOffset(2);
ACT.etc.cal.temp_z_offset = tempOffset(3);
% ---------------------------------------------------------
% Calibrate the data
